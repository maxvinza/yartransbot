...
telebot.apihelper.proxy = {'https': 'socks5://telegram:telegram@qzkjd.teletype.live:443'}
bot = telebot.TeleBot('KEY')

@bot.message_handler(commands=['start', 'help'])
def send_welcome(message):
    bot.reply_to(message, "Введите номер интересующего маршрута вида а23 или автобус23, тр1, тм1, мт96  ")

@bot.message_handler(func=lambda message: True)
def echo_all(message):
    input_mess = razbor_msg(message.text)
...
def stand_replace(url):
    ...
    htext = (htext.replace("К списку маршрутовнп - Низкопольное ТСпнп - Полунизкопольное ТСм/моб - Высокопольное ТС, оборудованное для перевозки маломобильных групп населениянул. - ТС следует в/из депо",""))
    htext = (htext.replace("===","\n"))
    return htext
...
def razbor_td(tr):
    ...
    if len(textl) > 2:
        return get_text(str(textl[1])) + " - " + get_text(str(textl[2])) + " - " + get_text(str(textl[3])) + " - " + str(make_ts(str(textl[5])))  + "===" #str(make_ts(str(textl[3]))) + "\n"
    elif len(textl) == 2:
